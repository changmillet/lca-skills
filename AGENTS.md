...
lastReviewedNote: "Reviewed for Skills #87: active remote skills use OAuth handoff and every current wrapper/CI/local-override contract pins immutable CLI 0.1.6 merge 72e27f3; authentication remains CLI-owned."
related:
  - .docpact/config.yaml
...
Review note, 2026-09-01: Skills #87 advances the active wrapper contract to published OAuth-capable CLI 0.1.6 and immutable release merge `72e27f3421937d8c07dcad77372fb3f548436e44`; no wrapper floats through `latest`, discovers a sibling checkout, or owns authentication logic.
...
- Node package execution is pinned to Node `24.19.0` and pnpm `11.24.0`; the default runtime is the exact published `@tiangong-lca/cli@0.1.6` and must never float through `@latest`.
